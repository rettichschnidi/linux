...
#define MT7628_ESW_FCT0	0x0008

#define MT7628_ESW_PFC1			0x0014
...
// test-only: restructure register and masks
...
#define MT7628_ESW_FPA	0x0084

#define MT7628_ESW_SOCPC		0x008c
...
#define MT7628_ESW_SGC	0x009c
#define MT7628_ESW_PCR0	0x00c0
...
#define MT7628_ESW_PCR1	0x00c4
#define PCR1_WT_DONE		BIT(0)
#define PCR1_RD_RDY		BIT(1)
#define PCR1_RD_DATA		GENMASK(31, 16)

// test-only: FPA1 vs FPA2
#define MT7628_ESW_FPA2	0x00c8
#define MT7628_ESW_FCT2	0x00cc

#define MT7628_ESW_SGC2	0x00e4
...
#define MT7628_ESW_BMU_CTRL	0x0110
...
struct mt7628_esw_priv {
	...
	struct esw_vlan vlans[MT7628_ESW_NUM_VLANS];
	struct esw_port ports[MT7628_ESW_NUM_PORTS];

	/* Protects against concurrent register r/w operations. */
	spinlock_t reg_rw_lock;
};

static void esw_rmw_raw(struct mt7628_esw_priv *priv, unsigned reg,
...
static void esw_rmw(struct mt7628_esw_priv *priv, unsigned reg,
		    unsigned long mask, unsigned long val)
{
	unsigned long flags;

	spin_lock_irqsave(&priv->reg_rw_lock, flags);
	esw_rmw_raw(priv, reg, mask, val);
...
static void esw_set_vlan_id(struct mt7628_esw_priv *priv,
...
static void esw_set_vmsc(struct mt7628_esw_priv *priv,
...
static void esw_set_pvid(struct mt7628_esw_priv *priv,
...
static void esw_set_gsc(struct mt7628_esw_priv *priv)
{
	esw_rmw(priv, MT7628_ESW_SGC,
		MT7628_ESW_GSC_BC_STROM_MASK << MT7628_ESW_GSC_BC_STROM_SHIFT,
		priv->bc_storm_protect << MT7628_ESW_GSC_BC_STROM_SHIFT);
	esw_rmw(priv, MT7628_ESW_SGC,
		MT7628_ESW_GSC_LED_FREQ_MASK << MT7628_ESW_GSC_LED_FREQ_SHIFT,
		priv->led_frequency << MT7628_ESW_GSC_LED_FREQ_SHIFT);
}

static unsigned esw_get_port_disable(struct mt7628_esw_priv *priv)
{
	unsigned reg;
...
static void esw_set_port_disable(struct mt7628_esw_priv *priv,
...
//	printk("%s (%d): disable_mask=%08x\n", __func__, __LINE__, disable_mask); // test-only
	old_mask = esw_get_port_disable(priv);
	changed = old_mask ^ disable_mask;
	enable_mask = old_mask & disable_mask;

	/* enable before writing to MII */
	esw_rmw(priv, MT7628_ESW_POC0,
...
// test-only: name
static int esw_apply_config(struct mt7628_esw_priv *priv)
{
	u8 disable = 0;
	u8 doubletag = 0;
	u8 en_vlan = 0;
	...
	for (i = 0; i < MT7628_ESW_NUM_VLANS; i++) {
		...
		esw_set_vlan_id(priv, i, vid);
		esw_set_vmsc(priv, i, vmsc);
	}
	...
	if (!priv->global_vlan_enable) {
		 ...
		 */
		esw_set_vlan_id(priv, 0, 0);
		esw_set_vmsc(priv, 0, MT7628_ESW_PORTS_ALL);
	}
...
static enum dsa_tag_protocol mt7628_get_tag_protocol(struct dsa_switch *ds,
						     int port)
{
...
static int mt7628_setup(struct dsa_switch *ds)
{
	...
	writel(0x00405555, base + MT7628_ESW_PFC1);
	writel(0x00007f7f, base + MT7628_ESW_POC0);
	writel(0x00007f7f, base + MT7628_ESW_POC2);	/* disable VLAN */
	writel(0x0002500c, base + MT7628_ESW_FCT2);
	/* hashing algorithm=XOR48, aging interval=300sec */
	writel(0x0008a301, base + MT7628_ESW_SGC);
	writel(0x02404040, base + MT7628_ESW_SOCPC);

	/* Ext PHY Addr=0x1f */
	writel(0x3f502b28, base + MT7628_ESW_FPA2); // FPGA1 from U-Boot ????
	writel(0x00000000, base + MT7628_ESW_FPA);
	/* 1us cycle number=125 (FE's clock=125Mhz) */
	writel(0x7d000000, base + MT7628_ESW_BMU_CTRL);
	...
	// test-only: get port_disable from DSA ports
	// test-only
	 ...
	 * default.
	 */
	// test-only: use port_map from original driver to support all ports
	esw_rmw(priv, MT7628_ESW_SGC2,
...
static const struct dsa_switch_ops mt7628_switch_ops = {
	.get_tag_protocol	= mt7628_get_tag_protocol,
	.setup			= mt7628_setup,
...
static int mt7628_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, irq, esw_interrupt, 0, "esw", priv);
	if (ret < 0) {
